...
Runs as an asyncio task inside the FastAPI process (see app/main.py lifespan).
A domain write appends its outbox event dict in the SAME store.mutate() call
as the change it describes; this worker only has to publish already-durable
entries, so a RabbitMQ outage delays delivery (entries stay unpublished and
get retried) instead of losing the event.
"""
